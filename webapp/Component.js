        ...
        return UIComponent.extend("zatualizacaocadastral.Component", {
            ...
            executeODataRead : function(){
                ...
                oModel.read("/headerSet('99999999')", {
                  ...
                  oJSONModel.setProperty('/dependentes', oData.dependenteSet);
                ...
                oModel.read("/paisSet", {
                  ...
                  oModel.read("/grauParentescoSet", {
                    success: jQuery.proxy(function(oData, response) {

                    var grauParentescoSet = [];
                    var entitySet = [];
                    var CodigoGrauParentesco;
                    var DescricaoGrauParentesco;    
                    ...
                    for (let x = 0; x < arrayLength; x++) {
                        CodigoGrauParentesco    = entitySet.results[x].CodigoGrauParentesco;
                        DescricaoGrauParentesco = entitySet.results[x].DescricaoGrauParentesco;

                        grauParentescoSet.push({
                            CodigoGrauParentesco    : CodigoGrauParentesco,
                            DescricaoGrauParentesco : DescricaoGrauParentesco
                        });
                    }
                   oJSONModel.setProperty('/grauParentescoSet', grauParentescoSet);   
                   oJSONModel.setProperty('/isBusy', false); 
                   }, this),
                       error: function(oError) {
                          oJSONModel.setProperty('/isBusy', false); 
